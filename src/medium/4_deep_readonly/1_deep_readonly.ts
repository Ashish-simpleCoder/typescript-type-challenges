...
// this will not work on Union types
// type DeepReadonly<T> = keyof T extends never ? T : { readonly [Key in keyof T]: DeepReadonly<T[Key]> }

// first case will fail
// type DeepReadonly<T> =  { readonly [Key in keyof T]: T[Key] extends Record<any, any> ? DeepReadonly<T[Key]>: T[Key] }

// we need to use type unknown for value type 
type DeepReadonly<T> = {
  readonly [Key in keyof T]: T[Key] extends Record<keyof any, unknown> | unknown[]
    ? DeepReadonly<T[Key]>
    : T[Key];
};
